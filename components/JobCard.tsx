...
import { IconBuildingSkyscraper, IconClockHour2, IconFileCertificate, IconFileDescription, IconLink } from '@tabler/icons-react';
import { buttonClassName, sectionClassName } from '@/utils/tagStylesConfig';
...
const JobCard = ({ job }: { job: JobData }) => {
  ...
  return (
    <Card className="bg-muted flex flex-col gap-3">
      <CardHeader>
         <JobInfo text={date} icon={<CalendarDaysIcon className='stroke-primary-accent stroke-1'/>} className='justify-end  '  /> 
        ...
        ?<a
             ...
             <IconLink stroke={1} className='stroke-primary-accent'/> {job.company}
            </a>
        : job.company
          } 
      ...
      <CardContent className="flex gap-2 flex-col flex-1 ">
        ...
        <section className='flex justify-between'>
          <JobInfo icon={<IconClockHour2 stroke={1}  className='stroke-primary-accent'/>} text={job.mode} />
...
<JobInfo icon={<MapPin  className='stroke-primary-accent stroke-1'/>} text={job.location} />
      ...
      <CardFooter className="flex gap-4 justify-end border-t-2 border-primary-accent pt-5">
        <Button asChild className={'border-2 border-primary-accent bg-transparent rounded-sm'} variant='outline'>
